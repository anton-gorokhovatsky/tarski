...
(() => {
  ...
  const kineticTextNodes = Array.from(document.querySelectorAll('.superlead p'));
  const reduceMotionQuery = window.matchMedia('(prefers-reduced-motion: reduce)');
  ...
  const setKineticVars = (x, y) => {
    root.style.setProperty('--kinetic-x', x.toFixed(3));
    root.style.setProperty('--kinetic-y', y.toFixed(3));
  };

  const resetKineticVars = () => {
    targetX = 0;
    targetY = 0;
    currentX = 0;
    currentY = 0;
    root.style.setProperty('--kinetic-scroll', '0');
    setKineticVars(0, 0);
  };

  const canUseKinetic = () => pointerQuery.matches && !reduceMotionQuery.matches;

  const wrapKineticText = (element) => {
    const source = element.textContent;

    if (!source || element.dataset.kineticSource === source) return;
    ...
    source.split(/(\s+)/).forEach((part) => {
      ...
      const profile = wordProfiles[wordIndex % wordProfiles.length];
      const span = document.createElement('span');

      span.className = 'kinetic-word';
      span.style.setProperty('--word-x', profile.x);
      span.style.setProperty('--word-y', profile.y);
      span.style.setProperty('--word-scroll', String(-3 - Math.min(wordIndex, 8) * 0.45));
      span.textContent = part;
      fragment.append(span);
      wordIndex += 1;
    });

    element.dataset.kineticSource = source;
    element.replaceChildren(fragment);
  };

  const wrapAllKineticText = () => {
    kineticTextNodes.forEach(wrapKineticText);
  };

  const tick = () => {
    currentX += (targetX - currentX) * 0.16;
    currentY += (targetY - currentY) * 0.16;
    setKineticVars(currentX, currentY);
  ...
  const handleScroll = () => {
    ...
    const coverDistance = Math.max(window.innerHeight * 0.9, 1);
    const progress = Math.min(Math.max(window.scrollY / coverDistance, 0), 1);
    root.style.setProperty('--kinetic-scroll', progress.toFixed(3));
  };
  ...
  window.addEventListener('tarski:languagechange', () => {
    window.requestAnimationFrame(wrapAllKineticText);
  });

  [reduceMotionQuery, pointerQuery].forEach((query) => {
